function draw_arrow(self, anchor, direction, context)
% Draw an arrow
%
% function draw_arrow(self, anchor, direction, context)
%
...
% direction= vector in which the arrow points, not necessarily a unit
% vector
...
%
    ...
    dim=length(anchor);
    if dim<3
        anchor(end+1)=0; direction(end+1)=0;
    end
    if dim<2
        anchor(end+1)=0; direction(end+1)=0;
    end
    anchor=reshape(anchor,3,1);
    direction=reshape(direction,3,1);
    c1=anchor; c2=anchor+direction;
    ...
    function draw_arrow1(self, anchor, direction, options)
        o=private_arrow(anchor,anchor + direction);
        if isfield(options,'color')
            color = options.color;
            set(o,'EdgeColor',color,'FaceColor',color,'FaceLighting','none');
        end
    end
    
    function [h,yy,zz] = private_arrow(varargin)
