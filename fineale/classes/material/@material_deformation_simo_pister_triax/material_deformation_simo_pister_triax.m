classdef material_deformation_simo_pister_triax < material_deformation_triax
    % Class that represents triaxial Simo-Pister (1984) isotropic hyperelastic material.
    %
    ...
    methods
        ...
        function self = material_deformation_simo_pister_triax(Parameters)
            % Constructor.
            % Parameters:
            % none
            %
        ...
        function [out, newms] = state(self, ms, context)
            ...
            b=Fn1*Fn1';
            J=det(Fn1);
            sigma = (self.mu/J) * (b - eye(3,3)) + (0.5*self.lambda *(J-1/J)) * eye(3,3);
            ...
            function out =get_var()
                switch context.output
                    ...
                    case'strain_energy'
                        C=Fn1'*Fn1;
                        out = self.mu/2*(trace((C- eye(3,3))/2)-log(J)) + self.lambda/4*(J^2-2*log(J)-1);
                    case '2ndPK'
                        invF1=inv(Fn1);
