% Compute the displacement norm for the inf-sup numerical test.
%
% function Sh = infsup_Sh (self, assembler, geom, u)
%
function Sh = infsup_Sh (self, assembler, geom, u)
fes = self.fes;% grab the finite elements to work on
    ...
    for i=1:size(conns,1)
        ...
        for j=1:npts
            ...
            B = self.hvgradmat(self,Ns{j},Ndersp*Rm,c,Rm);% strain-displacement
            Ke = Ke + B'*B * (Jac * w(j));
        end
        assemble_symmetric(assembler, Ke, dofnums);
    end
    Sh = make_matrix (assembler);
end
...
%             if (Jac<=0),error('Non-positive Jacobian');end
%             B = self.hvgradmat(self,Ns{j},Ndersp,c,Rm);% strain-displacement
