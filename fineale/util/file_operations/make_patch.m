function make_patch (PatchfileName,varargin)
% Make a file patch.
% 
% function make_patch (PatchfileName,varargin)
%     
%
% Creates a plain text file using mlar, the plaintext archiver for Matlab,
% storing in it all the files supplied as arguments.
%
% To extract the files stored in the patch, run it as a Matlab script.
% 
% The function may be run without arguments.  It will prompt the user for
% the needed information.
% 
%
% See also: mlar
%
% Copyright(C) 2007, 2008, Petr Krysl
    if (~exist('PatchfileName','var'))
%         PatchfileName =uigetfile('Prompt','Pick name for the archive',...
%             'NumFiles',1);
        [FileName,PathName,FilterIndex] = uiputfile('*','Pick name for the archive') ;
        if (FileName==0)
            warning('Action canceled: Pick name for the archive');
            return;
        end
        PatchfileName =[PathName FileName];
    end
    if (nargin<=1)
        files =uipickfiles('Prompt','Pick files to archive',...
            'NumFiles',[]);
        if (isempty(files ))
            warning('Action canceled: Pick files for the archive');
            return;
        end
        varargin=files;
    end
    if (~strcmp([strrep(PatchfileName,'.m','') '.m'],PatchfileName))
        PatchfileName =[PatchfileName,'.m'];
    end
    [s,mess,messid]=copyfile(which ('mlar'),PatchfileName,'f');
    if ~s
        error([PatchfileName ': ' mess]);
        return;
    end

    for j=1:length(varargin)
        varargin{j} =strrep(varargin{j},pwd,'.');
    end
    
    disp (['Archive file: ' PatchfileName])
    mlar(PatchfileName,varargin{:});
end
...
function out = uipickfiles(varargin)
    ...
    file_picks = {};
    full_file_picks = {};
    dir_picks = struct('name',{},'date','','bytes',[],'isdir',[],'datenum',0);
    ...
    removebut = uicontrol('Style','pushbutton',...
        'Position',[290 205 80 20],Font,...
        'String','Remove',...
        'Enable','off',...
        'Callback',@remove);
    moveupbut = uicontrol('Style','pushbutton',...
        'Position',[290 175 80 20],Font,...
        'String','Move Up',...
        'Enable','off',...
        'Callback',@moveup);
    movedownbut = uicontrol('Style','pushbutton',...
        'Position',[290 145 80 20],Font,...
        'String','Move Down',...
        'Enable','off',...
        'Callback',@movedown);

    uicontrol('Position',[10 380 250 16],Font,...
        'Style','text',...
        'String','Current Directory',...
        'HorizontalAlignment','center')
    dir_popup = uicontrol('Style','popupmenu',...
        'Position',[10 335 250 20],Font,...
        'BackgroundColor','w',...
        'String',path_cell(end:-1:1),...
        'Value',1,...
        'Callback',@dirpopup);
    hist_cm = uicontextmenu;
    pathbox = uicontrol('Position',[10 360 250 20],Font,...
        'Style','edit',...
        'BackgroundColor','w',...
        'String',current_dir,...
    ...
    uicontrol('Position',[10 425 80 16],Font,...
        'Style','text',...
        'String','File Filter',...
        'HorizontalAlignment','left')
    uicontrol('Position',[100 425 160 16],Font,...
        'Style','text',...
        'String','Reg. Exp. Filter',...
        'HorizontalAlignment','left')
    showallfiles = uicontrol('Position',[270 405 100 20],Font,...
        'Style','checkbox',...
        'String','Show All Files',...
        'Value',0,...
        'HorizontalAlignment','left',...
        'Callback',@togglefilter);
    filter_ed = uicontrol('Position',[10 405 80 20],Font,...
        'Style','edit',...
        'BackgroundColor','w',...
        'String',filter,...
        'HorizontalAlignment','left',...
        'Callback',@setfilspec);
    refilter_ed = uicontrol('Position',[100 405 160 20],Font,...
        'Style','edit',...
        'BackgroundColor','w',...
        'String',re_filter,...
        'HorizontalAlignment','left',...
    ...
    viewfullpath = uicontrol('Style','checkbox',...
        'Position',[380 335 230 20],Font,...
        'String','Show full paths',...
        'Value',show_full_path,...
        'HorizontalAlignment','left',...
        'Callback',@showfullpath);
    remove_dupes = uicontrol('Style','checkbox',...
        'Position',[380 360 230 20],Font,...
        'String','Remove duplicates (as per full path)',...
        'Value',nodupes,...
        'HorizontalAlignment','left',...
        'Callback',@removedupes);
    uicontrol('Position',[380 405 350 20],Font,...
        'Style','text',...
        'String','Selected Files',...
        'HorizontalAlignment','center')
    uicontrol('Position',[270 60 100 50],Font,'String','Done',...
        'Callback',@done);
    uicontrol('Position',[270 10  100 30],Font,'String','Cancel',...
        'Callback',@cancel);

    if ~isempty(prop.numfiles)
        uicontrol('Position',[380 385 350 16],Font,...
            'Style','text',...
            'String',numstr,...
            'ForegroundColor','r',...
            'HorizontalAlignment','center')
