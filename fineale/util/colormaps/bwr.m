function b = bwr(Number_of_colors)
    % Blue to white to red color map.
    %
    %     b = bwr(Number_of_colors)
    %
    ...
    if (~exist('Number_of_colors','var'))
        Number_of_colors=256;
    end
    color3 = [1 0 0];
    color2 = [1 1 1];
    color1 = [0 0 1];
    b=usercolormap13(color1,color2,color3);

    function C = usercolormap13(varargin)
        ...
        %% interpolation %%
        lenC0 = size(C0,1);
        step = linspace(1,Number_of_colors,lenC0);

        C = zeros(Number_of_colors,3);
        for k = 1:3
            foo = interp1(step',C0(:,k),1:Number_of_colors);
            C(:,k) = foo';
        end
