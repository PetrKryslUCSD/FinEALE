% Axially symmetric analysis of a shaft: axis(1) + point(2) + self-equilibrated 
disp('Axially symmetric analysis of a shaft: axis(1) + point(2) + self-equilibrated');
...
[fens,fes,groups,edge_fes,edge_groups]=targe2_mesher({...
    ...
    ['m-ctl-point 1 xy ' num2str(R1) ' ' num2str(L1+Rf) ' near ' num2str(mesh_size/1) ' influence ' num2str(mesh_size/10)],...
...
efemm2 = femm_deformation_linear (struct ('material',mater, 'fes',subset(edge_fes,edge_groups{5}),...
    'integration_rule',gauss_rule (struct('dim',1,'order', 2))));
efemm1 = femm_deformation_linear (struct ('material',mater, 'fes',subset(edge_fes,edge_groups{1}),...
    'integration_rule',gauss_rule (struct('dim',1,'order', 2))));
% Geometry
...
u   = u*0; % zero out
% Apply EBC's
ebc_fenids=fenode_select (fens,struct('box',[0 0 0 0],'inflate',R1/10000));
...
F = F + distrib_loads(efemm2, sysvec_assembler, geom, u, fi, 2);
fi=force_intensity(struct('magn',[0;-forc/(pi*R1^2)]));
F = F + distrib_loads(efemm1, sysvec_assembler, geom, u, fi, 2);
% Solve
