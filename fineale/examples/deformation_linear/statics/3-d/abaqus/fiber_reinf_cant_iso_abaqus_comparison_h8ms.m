% Isotropic-material cantilever.
function fiber_reinf_cant_iso_abaqus_comparison_h8ms
...
%         'Rm',Rm,'integration_rule',gauss_rule(struct('dim',3,'order',2)), 'stab_fraction',0.2,'weights',1-2*nu));
%     eltyd(eix).surface_integration_rule=gauss_rule(struct('dim',2, 'order',2));
%     eix=eix+1;
    
% eltyd(eix).description ='H8MSNIS';
    % eltyd(eix).mf =@(ref)H8_block(a,b,t,2*ref*na,2*ref*nb,2*ref*nt);
    % eltyd(eix).femmf =@(fes)femm_deformation_linear_h8msnis(struct('fes',fes,'material',mater,'Rm',Rm,'nconstrained',1, 'stab_fraction',0.2));
    % eltyd(eix).surface_integration_rule=gauss_rule(struct('dim',2, 'order',2));
    % eix=eix+1;
    ...
    % eltyd(eix).mf =@(ref)H8_block(a,b,t,2*ref*na,2*ref*nb,2*ref*nt);
    % eltyd(eix).femmf =@(fes)femm_deformation_linear_nice_1pt_h8_gsri(struct('fes',fes,'material',mater,...
    %     'Rm',Rm));
    % eltyd(eix).surface_integration_rule=gauss_rule(struct('dim',2, 'order',2));
    % eix=eix+1;
    ...
    for eix = 1:length(eltyd)
        ...
        for ref = refs
            ...
            if (graphics )
                ...
                model_data.postprocessing.stress_component=5;'vonmises';
                %                 model_data=deformation_plot_stress(model_data)
                colorbar_context.interpreter='latex';;
                colorbar_context.position=[0.71, 0.1, 0.025, 0.5];;
                colorbar_context.label=['$\|u\|$'];;
                colorbar_context.fontname= 'Times';;
                colorbar_context.fontsize= 24;
                colorbar_context.interpreter= 'latex';
                model_data.postprocessing.colorbar_context=colorbar_context;;
                model_data=deformation_plot_deformation(model_data);
                axis off
                            draw_axes (model_data.postprocessing.gv,struct('origin',[0,4*b,0],'length',2*b,  'fontsize',24));
