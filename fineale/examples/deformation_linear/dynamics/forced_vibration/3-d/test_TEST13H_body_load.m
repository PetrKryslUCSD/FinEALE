...
% simply-supported on the circumference. The support motion is converted to
% a body load, and the problem is solved for the displacement relative to
% the motion of the support.
...
function test_TEST13H_support_motion
    ...
    %%
    %
    % Define the body load
    clear body_load
    body_load.force=@(frequency)[0;0;(-1)*(2*pi*frequency)^2*rho*0.001*pu.M];
    body_load.fes= fes;% set of finite elements for the interior of the domain
    body_load.integration_rule = gauss_rule (struct('dim', 3, 'order', 2));
