%% Suddenly applied harmonic load on a bar; 2-D axially symmetric model with implicit integration
%

%%
% Link to the  <matlab:edit('pub_sin_rod_impl_2D_axi') m-file>.

%% Description
% The structure is a clamped bar that consists of two materials, a short
% section at the free end of stiff material,  and a long section of
% flexible material adjacent to the clamped end (see Figure 1).
%%
...
% <img src="../docs/pub_sin_rod_2D_axi.png" width = "60%">
% </td></tr>
...
%% Solution
%
function pub_sin_rod_impl_2D_axi
...
% be employed for the two-coordinate (axially symmetric) representation.
    ...
    region.nu=nu1;
    region.rho=rho1;
    rl1=fe_select (fens,fes,...
        struct('box',[-inf,inf,0,L1],'inflate',tolerance));
...
% The integration rule is two-dimensional as we integrate in the section of the rod.
    ...
    essential.node_list = fenode_select (fens,...
        struct('box',[-inf,inf,L,L],'inflate',tolerance));
    ...
    % method can deal with the given time step as it is above the critical
    % time step for  explicit methods (as determined by the stiff portion
    % of the rod).
    model_data =...
        deformation_linear_direct_implicit_TRAP_Rayleigh(model_data);
    ...
    plot(model_data.geom.values(:,2),model_data.u.values(:,2),...
        'r+', 'linewidth',2);
    set(gca,'xlim',[-eps,L]);
    ...
    % of  either the 1D or the 3-D model.  It is considerably cheaper than
    % the 3-D model though.
end