%% Suddenly applied harmonic load on a bar; 1-D model with centered-difference  rule
%

%%
% Link to the  <matlab:edit('pub_sin_rod_expl_1D') m-file>.
%
...
%% 
% The equation of motion will be integrated with the center-difference
% explicit rule, and therefore the time step will be determined by the
% solver.
...
tend/length(tipu)
%% 
% seconds. The time step length was determined  from the Courant condition,
% in this case driven by the stiff portion of the rod. Using the formula
% for the traversal of the length of the element by  the longitudinal wave
% we get  an estimate of the critical time step of
(L/n/sqrt(E1/rho1))
%% 
% It may be appreciated that the Courant condition  gives a close estimate of
% the actual time step chosen by the solver  from the largest eigenvalue of
