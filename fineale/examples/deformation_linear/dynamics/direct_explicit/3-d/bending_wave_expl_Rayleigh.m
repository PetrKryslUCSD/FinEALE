function bending_wave_expl_Rayleigh
    % Bending wave in a clamped beam. Centered-difference integration with Rayleigh damping.
    graphics = false;
    igraphics=(1:250:40000000);
    plots = true;
    % Parameters:
    ...
    model_data.implicit_solve = ~true;
    model_data.tend = tend;
    model_data.step_reduction = 0.29;
    ...
    % Solve
    tic; model_data =deformation_linear_direct_explicit_CD_Rayleigh(model_data);
    toc
    ...
    function output(t, model_data)
        ...
        elseif plots
            ...
            if (~isempty(igraphics) && (igraphics(1)==round(t/model_data.dt)))
                ...
                plot(t,corneru(3,end),'bx','Markersize',4); hold on
                igraphics= igraphics(2:end);
            end
        end
    end
