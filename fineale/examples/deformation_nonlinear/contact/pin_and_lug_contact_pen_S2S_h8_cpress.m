...
function  pin_and_lug_contact_pen_S2S_h8_cpress
...
nL=15;nH=4;nW=15;na=25;tolerance=min([L,W,a,H])/1e5;
tol = a*10e-7;
...
while 1
    ...
    K = Ks + Ksp + Kg;
    du = scatter_sysvec(du, K\(gl+Fr+Fl));
...
draw_colorbar(gv, struct('colormap',dcm.colormap,'position',[0.8 0.15 0.025 0.7],...
    'minmax',dcm.range,'label','cpress'));    
xlabel('X');
ylabel('Y');
zlabel('Z');
hold on
