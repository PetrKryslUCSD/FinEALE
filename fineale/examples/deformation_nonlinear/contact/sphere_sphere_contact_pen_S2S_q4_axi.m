% Deformable sphere squashed between two rigid spheres.
% Axially symmetric model, due to symmetry only one half of the cross-section  is discretized.
% Penalty contact approach.
function  Sphere_plane_contact_pen_S2S_q4_axi
disp('Sphere squashed between two rigid spheres. Penalty contact algorithm.');
...
na=35;tolerance=min([a])/1e5;
sscale=1.0;
penalty = 5000*E;
utol=1e-15*U.M;
augtol=1e-12;
surface_data.radius=[1.2*a];
surface_data.center=[0,a+0.9*surface_data.radius];

    function  [penetration,normal] = get_penetration (surface_data,X,U)
        p=X-surface_data.center;
        normal = p/norm(p);
        penetration = surface_data.radius - dot(p,normal) - dot(U,normal);
    end
