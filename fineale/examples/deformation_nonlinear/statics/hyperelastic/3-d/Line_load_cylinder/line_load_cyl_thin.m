...
function res=line_load_cyl_thin
...
while (incr <= nincr)
    ...
    while 1
        ...
        fi= force_intensity(struct('magn',t*[0;0;-0.067]));
...
%         draw(sfemm,gv, struct ('x', geom, 'u', 0*u1,'facecolor','none'));
%         draw(sfemm,gv, struct ('x', geom, 'u', u1,'facecolor','y'));
%         interact(gv); pause(1);
