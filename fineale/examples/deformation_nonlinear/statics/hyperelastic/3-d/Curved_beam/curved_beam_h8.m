function [u1s,lambdas]=curved_beam_h8
%%
...
[fens,fes]= H8_block(h,ang/180*pi,b,1,nel,1);
...
while (incr <= nincr)
   lambda = incr* tup / nincr;
    ...
    if graphics
        gv=reset(clear(gv,[]),[]);
        draw(sfemm,gv, struct ('x', geom, 'u', 0*u,'facecolor','none', 'linewidth',2));
        draw(sfemm,gv, struct ('x', geom, 'u', u,'facecolor','y', 'linewidth',2));
        camset (gv,Cam);
        interact(gv);
