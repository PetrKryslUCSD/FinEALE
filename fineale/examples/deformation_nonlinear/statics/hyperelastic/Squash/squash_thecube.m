function squash_thecube(ntmult)
    if ~exist('ntmult','var'), ntmult=4; end
...
function [fens,fes] = h8T10MS(nt)
        [fens,fes]=T10MS_blocka(1.0, 1.0, 1.0, nt(1),nt(2),nt(3));
    end
    ...
    eltyd(eix).mf =@h8T10MS;
    eltyd(eix).blf =@(fes)femm_deformation_nonlinear_t10ms(struct('fes',fes,'material',mater,'integration_rule',tet_rule(struct('npts',4))));
