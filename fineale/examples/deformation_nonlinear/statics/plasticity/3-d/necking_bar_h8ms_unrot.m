function necking_bar_h8ms_unrot
disp('Necking bar: Unrotated J2 plasticity. Mean-strain hexahedron.');
% Parameters:
...
E=206.900*u.GIGA*u.PA;
nu= 0.29;
rho  = 8930*u.KG/u.M^3;
%   sigma_y=0.3*u.MEGA*u.PA; Hi=0.7*u.MEGA*u.PA;
sigma_y=0.45*u.GIGA*u.PA; sigma_res=0.715*u.GIGA*u.PA;
delta_expon=16.93; K_lin=0.12924*u.GIGA*u.PA; beta=1;
Estab= K_lin+delta_expon*(sigma_res-sigma_y);
...
scale=1;
stressscale=0.001;
epscale=0.0005*scale;
graphics = ~true;
igraphics=10;
plots = true;
maxdu_tol = 0*Length/1e4;
maxbal_tol =pi*Radius^2*sigma_y/1e9;
umag=Length/4;
...
stabprop = property_deformation_neohookean(struct('E',Estab,'nu',nu));
...
if (1)% Choose hexahedral mesh
    ...
    femm = femm_deformation_nonlinear_h8msgso(struct ('material',mater,'stabilization_material',stabmater, 'fes',fes, ...
...
% Observer function to be called when convergence is reached.
    function load_increment_observer(lambda,model_data)
        ...
        Elongation=[ Elongation,mean(model_data.un1.values(movingl,3))];
        Neck_radius=[ Neck_radius,Radius+mean(model_data.un1.values(corner,1))];
        Rx=[Rx,sum(model_data.reactions.values(movingl,1))];
        Lambdas=[Lambdas,lambda];
        if (~graphics)
            %  plot(Lambdas,(Radius+Ux)/Radius,'gv-')
            plot(Elongation,Neck_radius,'gv-')
            pause (0.1)
        end
