...
% (C)  2015, Petr Krysl
%
% Layered wall.  Heat-flux boundary conditions on both sides.
function pnpLayeredWallqboth
...
node2dof=zeros(1,N);
node2dof([2,5,3,6,4,1]) =1:N_f;% number the free degrees of freedom
...
for j=1:size(connL2,1) % right bboundary
    he=norm(diff(x(connL2(j,:),:)));
    Leq=-(-qnbar)*he*Dz/2*[1;1];
    edof= node2dof(connL2(j,:));%element degree of freedom array
    msk=(edof<=N_f);% Boolean mask array: only free dofs
    % Assemble elementwise load vector to the global vector
    Lg(edof(msk))=Lg(edof(msk))+Leq(msk);
end
