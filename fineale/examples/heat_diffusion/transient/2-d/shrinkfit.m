% Temperature equalization  in shrink-fitting.
function shrinkfit
...
[fens,fes,groups,edge_fes,edge_groups]=targe2_mesher({...
    ...
    function observer (t,  model_data)
        ...
        dcm=data_colormap(struct('range',[Ts,Tt],'colormap',jet));
        ...
        draw(model_data.region{1}.fes, gv, struct ('x',model_data.geom, 'u',0*model_data.geom,...
            'facecolor','none'));
        draw(model_data.region{2}.fes, gv, struct ('x',model_data.geom, 'u',0*model_data.geom,...
            'facecolor','none'));
        draw(model_data.region{1}.fes, gv, struct ('x',geomT, 'u',0*geomT,...
            'colorfield',colorfield));
        draw(model_data.region{2}.fes, gv, struct ('x',geomT, 'u',0*geomT,...
            'colorfield',colorfield));
        labels X Y T ;
