function [fens,fes] = T3_ablock(Length,Width,nL,nW,options)
% Mesh of T3 elements of a rectangle.  Alternate orientation.
%
% function [fens,fes] = T3_ablock(Length,Width,nL,nW,options)
%
% The range is <0,Length> x <0,Width>
...
% 
% Examples: 
%     [fens,fes] = T3_ablock(3.1,2.2,3,2,[]);
...
function nn=node_numbers1(i,j,nL,nW) %#ok<INUSD>
    f=(j-1)*(nL+1)+i;
    nn=[f (f+1) f+(nL+1)+1];
    return;
end
function nn=node_numbers2(i,j,nL,nW) %#ok<INUSD>
    f=(j-1)*(nL+1)+i;
