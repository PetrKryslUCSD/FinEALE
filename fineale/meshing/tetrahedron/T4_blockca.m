function [fens,fes] = T4_blockca(Length,Width,Height,nL,nW,nH)
    % Tetrahedral mesh of a rectangular block; orientation 'ca'.
    %
    % function [fens,fes] = T4_blockca(Length,Width,Height,nL,nW,nH)
    %
    ...
    %
    % Examples:
    %         [fens,fes] = T4_blockca(2,3,4, 2,6,4);
    %         figure; drawmesh({fens,fes},'fes','facecolor','b'); hold on
    %
    % See also: T4_blockx,  T4_blocka,  T4_blockb,  T4_blockcb
    [fens,fes] = T4_blockx(linspace(0,Length,nL+1)',linspace(0,Width,nW+1)',linspace(0,Height,nH+1)','ca');
end
