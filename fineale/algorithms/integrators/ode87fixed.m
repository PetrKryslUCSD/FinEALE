function [tout,xout] = ode87fixed(odefun,tspan,x0,options,varargin)
% Fixed-step 8th-order accurate Runge-Kutta integrator. 
%
% ODE87FIXED  is a realization of explicit Runge-Kutta method. 
...
% expected is O(h^9).  
% This requires 13 function evaluations per integration step. Warning: this
% file was modified to disallow time step change.  Was the order affected?
% Petr Krysl, 2009
%
...
%   [T,Y] = ODE87FIXED(ODEFUN,TSPAN,Y0) with TSPAN = [T0 TFINAL] integrates the
...
%   
%   [T,Y] = ODE87FIXED(ODEFUN,TSPAN,Y0,OPTIONS) solves as above with default
%   integration properties replaced by values in OPTIONS, an argument created
%   with the ODESET function. See ODESET for details. Commonly used options 
%   are scalar relative error tolerance 'RelTol' (1e-6 by default).
%   
%   [T,Y] = ODE87FIXED(ODEFUN,TSPAN,Y0,OPTIONS,P1,P2...) passes the additional
...
% Step control% Reduced to fixed-time step Integrator. Petr Krysl 2009
      if Error_step == 0.0
         Error_step = eps*10.0;
      end;
